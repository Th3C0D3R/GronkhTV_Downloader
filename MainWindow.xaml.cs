...
namespace GronkhTV_DL
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private bool InitWebDriver()
        {
            ...
            {
                if (ex.Message.StartsWith("session not created"))
                {
                    MessageBox.Show(ex.Message.Replace("session not created:", "").Trim());
                }
                return false;
            }
                ...
                if (stream.Qualities.StreamQualities.Count > 0)
                {
                    ...
                    if (sqdialog.ShowDialog() ?? false)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(url))
                        {
                            Player p = new();
                            p.Show();
                            p.Play(url, stream.video_length);
                        }
                    }
                }
            }
